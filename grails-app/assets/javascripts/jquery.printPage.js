(function( $ ){
    $.fn.printPage = function(options) {
        // EXTEND options for this button
        var pluginOptions = {
            attr : "href",
            url : false,
            message: "Espere mientras creamos el documento....."
        };
        $.extend(pluginOptions, options);

        this.on("click", function(){  loadPrintDocument(this, pluginOptions); return false;  });

        /**
         * Cargamos y mostramos el mensaje, llamada del iframe
         * @param {jQuery} el - Boton llamando el plugin
         * @param {Object} pluginOptions - opciones para impresiones
         */
        function loadPrintDocument(el, pluginOptions){
            $("body").append(components.messageBox(pluginOptions.message));
            $("#printMessageBox").css("opacity", 0);
            $("#printMessageBox").animate({opacity:1}, 300, function() { addIframeToPage(el, pluginOptions); });
        }
        /**
         * Mostramos el iframe dentro del documento tratando de ocultarlo no puede usar display:none
         * No se puede imprimir si el elemento no esta mostrado en pantalla
         * @param {jQuery} el - El boton llama el plugin
         * @param {Object} pluginOptions - opciones para el botón de impresión
         */
        function addIframeToPage(el, pluginOptions){

            var url = (pluginOptions.url) ? pluginOptions.url : $(el).attr(pluginOptions.attr);

            if(!$('#printPage')[0]){
                $("body").append(components.iframe(url));
                $('#printPage').bind("load",function() {  printit();  })
            }else{
                $('#printPage').attr("src", url);
            }
        }
        /*
         * Llame a la funcionalidad del navegador de impresión, IE necesita focus
         */
        function printit(){
            frames["printPage"].focus();
            frames["printPage"].print();
            unloadMessage();
        }
        /*
         * Ocultar y eliminar el cuadro de mensaje con un pequeño retraso
         */
        function unloadMessage(){
            $("#printMessageBox").delay(1000).animate({opacity:0}, 700, function(){
                $(this).remove();
            });
        }
        /*
         * Construir componentes HTML para este plugin
         */
        var components = {
            iframe: function(url){
                return '<embed id="printPage" name="printPage" src='+url+' style="position:absolute;top:0px; left:0px;width:0px; height:0px;border:0px;overfow:none; z-index:-1"></embed>';
            },
            messageBox: function(message){
                return "<div id='printMessageBox' style='position:fixed;top:50%; left:50%;text-align:center;margin: -60px 0 0 -155px;width:310px; height:120px; font-size:16px; padding:10px; color:#222; font-family:helvetica, arial;opacity:0;background:#fff url(data:image/gif;base64,) center 40px no-repeat;border: 6px solid #555;border-radius:8px; -webkit-border-radius:8px; -moz-border-radius:8px;box-shadow:0px 0px 10px #888; -webkit-box-shadow:0px 0px 10px #888; -moz-box-shadow:0px 0px 10px #888'>"+message+"</div>";
            }
        }
    };
})( jQuery );
